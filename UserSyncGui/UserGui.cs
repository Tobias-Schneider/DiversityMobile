...
namespace UserSyncGui
{
    public partial class UserGui : Form
    {
        ...
        private Serializer taxonRepSerializer;
        ...
        private void checkConnections()
        {
            ...
            try
            {
                ...
                //IRestriction r = RestrictionFactory.Eq(typeof(UserProfile), "_LoginName", "Schneider");
        ...
        private bool createProfile()
        {
            ...
            //IRestriction r = RestrictionFactory.Eq(typeof(UserProxy), "_LoginName", "Schneider");
            ...
            if (projects.Count > 0)
            {
                ...
                //4. Namen  holen
                DbConnection conn = taxonRepSerializer.CreateConnection();
                DbCommand comm = conn.CreateCommand();
                comm.CommandText = "SELECT [AgentName] FROM [DiversityMobile].[dbo].[IBFagents] WHERE AgentURI='" + profile.AgentURI + "'";
                conn.Open();
                DbDataReader reader = null;
                try
                {
                    ...
                    while (reader.Read())
                    {
                       profile.CombinedNameCache = reader.GetString(0);
                    }
                    conn.Close();
                }
                catch (Exception e)
        ...
        private void getDefinitionsListSynchronization()
        {
            if (this.startMobileConnection(true))
            {
                ...
                String sql;
                IList<ISerializableObject> list = new List<ISerializableObject>();
                this.setProgressInformation("Read Definitions from Database");
                try
                {
                    sql = @"SELECT * FROM [" + this.InitialCatalog + "].[dbo].[AnalysisProjectList] (" + this.projectID + ")";
                    list = repositorySerializer.Connector.LoadList(typeof(Analysis), sql);
                ...
                this.setProgressValue(8);
                MessageBox.Show("Analyses successfully loaded");
                ...
                {
                    this.setProgressInformation("Error: Loading AnalysisTaxonomicGroups");
                    connRepository.Close();
                }
                MessageBox.Show("AnalysisTaxonomicGroups successfully loaded");
                ...
                this.setProgressValue(12);
                try
                {
                    transferList.Load(typeof(Property), repositorySerializer);
                    MessageBox.Show(typeof(Property).ToString() + " successfully loaded");
                }
                catch (Exception e)
                {
                    MessageBox.Show("Error loading Properties: " + e.Message);
                }
                //try
                //{
                //    transferList.Load(typeof(CollEventImageType_Enum), repositorySerializer);
                //    MessageBox.Show(typeof(CollEventImageType_Enum).ToString() + " successfully loaded");
                //}
                //catch (Exception e)
                //{
                //    MessageBox.Show("Error loading CollEventImageType_Enum: " + e.Message);
                //}
                try
                {
                    transferList.Load(typeof(CollSpecimenImageType_Enum), repositorySerializer);
                    MessageBox.Show(typeof(CollSpecimenImageType_Enum).ToString() + " successfully loaded");
                }
                catch (Exception e)
                {
                    MessageBox.Show("Error loading CollSpecimenImageType_Enum: " + e.Message);
                }
                try
                {
                    transferList.Load(typeof(LocalisationSystem), repositorySerializer);
                    MessageBox.Show(typeof(LocalisationSystem).ToString() + " successfully loaded");
                }
                catch (Exception e)
                {
                    MessageBox.Show("Error loading LocalisationSystem: " + e.Message);
                }
                try
                {
                    transferList.Load(typeof(CollCircumstances_Enum), repositorySerializer);
                    MessageBox.Show(typeof(CollCircumstances_Enum).ToString() + " successfully loaded");
                }
                catch (Exception e)
                {
                    MessageBox.Show("Error loading CollCircumstances_Enum: " + e.Message);
                }
                try
                {
                    transferList.Load(typeof(CollUnitRelationType_Enum), repositorySerializer);
                    MessageBox.Show(typeof(CollUnitRelationType_Enum).ToString() + " successfully loaded");
                }
                catch (Exception e)
                {
                    MessageBox.Show("Error loading CollUnitRelationType_Enum: " + e.Message);
                }
                try
                {
                    transferList.Load(typeof(CollIdentificationCategory_Enum), repositorySerializer);
                    MessageBox.Show(typeof(CollIdentificationCategory_Enum).ToString() + " successfully loaded");
                }
                catch (Exception e)
                {
                    MessageBox.Show("Error loading CollIdentificationCategory_Enum: " + e.Message);
                }
                //transferList.Load(typeof(Property), repositorySerializer);
                //transferList.Load(typeof(CollEventImageType_Enum), repositorySerializer);
                //transferList.Load(typeof(CollSpecimenImageType_Enum), repositorySerializer);
                //transferList.Load(typeof(CollTaxonomicGroup_Enum), repositorySerializer);
                //transferList.Load(typeof(LocalisationSystem), repositorySerializer);
                //transferList.Load(typeof(CollCircumstances_Enum), repositorySerializer);//
                //transferList.Load(typeof(CollUnitRelationType_Enum), repositorySerializer);
                ...
                this.setProgressInformation("Transfering");
                ...
                foreach (ISerializableObject iso in orderedObjects)
                {
                    try
                    {
                        mobileDBSerializer.Connector.InsertPlain(iso);
                    }
                    catch (Exception)
                    {
                        try
                        {
                            if(iso.GetType().Equals(typeof(AnalysisTaxonomicGroup)))
                            {
                                ...
                                ISerializableObject isoStored=mobileDBSerializer.Connector.Load(iso.GetType(),r);
                            ...
                            mobileDBSerializer.Connector.UpdatePlain(iso);
                        }
                        catch (Exception ex)
                        {
                            MessageBox.Show("TransferError: " + iso.ToString() + " " + ex.Message);
                        }
                    }
                ...
                this.setProgressInformation("Transfer Complete");
        ...
        private void uploadListSynchronizsation()
        {
            if (this.startMobileConnection(false))
            {
                ...
                this.setProgressValue(10);
                String userNumber=this.profile.AgentURI.Replace(@"http://id.snsb.info/Agents/","");
                SNSBPictureTransfer snsb = new SNSBPictureTransfer(userNumber,this.projectID, this.mobileDBSerializer);
                ...
                AnalyzeSyncObjectList ansl = new AnalyzeSyncObjectList(syncList, mobileDBSerializer, repositorySerializer, syncDBSerializer,snsb);
                ansl.analyzeAll();
                ...
                {
                    //ConflictHandling ch = new ConflictHandling(lc.Iso, lc.Partner);
                    //ch.analyzeObjects();
                    //ch.printConflicts();
            ...
            if (repositoryConnected == false)
            {
                if (this.SqlAuth)
                {
                    ...
                    if (lf.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        Cursor.Current = Cursors.WaitCursor;
                        repositorySerializer = new MS_SqlServerIPSerializer(loginName, password, this.IPAddress, this.IPPort, this.InitialCatalog, String.Empty);
                        taxonRepSerializer = new MS_SqlServerIPSerializer(loginName, password, this.IPAddress, this.IPPort, this.TaxonNamesInitialCatalog, String.Empty);
                        Cursor.Current = Cursors.Default;
                    }
                    else
                    {
                ...
                {
                    Cursor.Current = Cursors.WaitCursor;
                    repositorySerializer = new MS_SqlServerWASerializier(this.IPAddress + "," + this.IPPort, this.InitialCatalog, String.Empty);
                    taxonRepSerializer = new MS_SqlServerWASerializier(this.IPAddress + "," + this.IPPort, TaxonNamesInitialCatalog, String.Empty);
                ...
                try
                {
                    ...
                    if (this.SqlAuth)
                        syncDBSerializer = new MS_SqlServerIPSerializer(loginName, password, this.IPAddress, this.IPPort, sync, String.Empty);
                    else
                        syncDBSerializer = new MS_SqlServerWASerializier(this.IPAddress + "," + this.IPPort, sync, String.Empty);

                    String syncIt = "[" + loginName + "].SyncItem";
                    String fieldSt = "[" + loginName + "].FieldState";
                    //String syncIt = "[Schneider].SyncItem";
                    //String fieldSt = "[Schneider].FieldState";
        ...
        {
            this.listBoxChooseProject.Items.Clear();
            this.listBoxChooseProject.Visible = true;
            this.buttonChoose.Visible = true;
            this.buttonChooseProject.Enabled = false;
            DbConnection repConn = this.repositorySerializer.CreateConnection();
            ...
            //sb.Append(" (Select ProjectID From ProjectUser Where LoginName='Schneider')");
            com.CommandText = sb.ToString();
            repConn.Open();
            ...
            while (reader.Read())
            {
                this.listBoxChooseProject.Items.Add(reader["ProjectID"] + ": " + reader["Project"]);
            }
        ...
        private void buttonGetTaxa_Click(object sender, EventArgs e)
        {
            ...
            int i;
            String[] sourceTablesFungi = { "TaxRef_DGfM_Fungi" };
        ...
        private void buttonGetDefinitions_Click(object sender, EventArgs e)
        {
            ...
            this.startProgressThread();
            this.setProgressValue(0);
            this.setActionInformation("Get Definitions");
            
            getDefinitionsListSynchronization();
        ...
        {
            SelectFieldDataForm sf = new SelectFieldDataForm(this.projectID, repositorySerializer);
            List<ISerializableObject> selection = new List<ISerializableObject>();
            if (sf.ShowDialog() == DialogResult.OK)
            {
                //foreach (KeyValuePair<Guid, ISerializableObject> kvp in sf.Selection)
                //{
                //    selection.Add(kvp.Value);
                //}
            ...
            this.startProgressThread();
            this.setProgressValue(0);
            this.setActionInformation("Download Field Data");
            
            getFieldDataListSynchronization(selection);
